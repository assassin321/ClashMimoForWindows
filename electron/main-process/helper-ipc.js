/**
 * Clash Mimo For Windows Helper Service IPC Client
...
const PIPE_NAME = '\\\\.\\pipe\\clashmeta-helper-service';
const SECRET_SEED = 'clashmeta-helper-service-secret-key-v1';
const MESSAGE_EXPIRY_SECS = 30;
const SERVICE_NAME = 'ClashMetaHelperService';

// 派生密钥（优先读取动态密钥，回退到硬编码）
function loadSecretKey() {
  // 尝试读取动态密钥
  const pd = process.env.ProgramData || 'C:\\ProgramData';
  const keyPath = path.join(pd, 'Clash Mimo For Windows', 'service-key');
  try {
...
function getHelperPath() {
  ...
  const possiblePaths = [
    ...
    path.join(process.resourcesPath || '', 'clashmeta-helper.exe'),
    // 开发环境路径
    path.join(app.getAppPath(), 'tools', 'clashmeta-helper.exe'),
    path.join(__dirname, '..', '..', 'tools', 'clashmeta-helper.exe'),
    path.join(__dirname, '..', '..', 'native', 'helper', 'clashmeta-helper.exe'),
  ];
...
async function installService() {
  ...
  if (!isAdmin()) {
    return {
      success: false,
      error: '安装服务需要管理员权限，请先退出 Clash Mimo For Windows，然后右键以管理员身份运行后再尝试安装服务。',
      needsAdmin: true
    };
  }
...
function cleanupLegacyServices() {
  const legacyNames = [
    'Clash Mimo For WindowsCoreService',
    'clashmetacoreservice',
    'clashmetacoreservice.exe'
  ];
...
async function uninstallService() {
  ...
  if (!isAdmin()) {
    return {
      success: false,
      error: '卸载服务需要管理员权限，请先退出 Clash Mimo For Windows，然后右键以管理员身份运行后再尝试卸载服务。',
      needsAdmin: true
    };
  }
