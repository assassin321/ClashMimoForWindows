        ...
        if (isWindows) {
          ...
          if (!perm || !perm.success || !perm.hasPermission) {
            ...
            if (isInstalled === false) {
              return { success: false, error: 'TUN 服务未安装，请在 TUN 设置页面安装服务，或以管理员身份运行 Clash Mimo For Windows' };
            }
            if (isInstalled && isRunning === false) {
              return { success: false, error: 'TUN 服务未运行，请在 TUN 设置页面启动服务，或以管理员身份运行 Clash Mimo For Windows' };
            }
            if (!isAdmin && !isRunning) {
              return { success: false, error: 'TUN 模式需要服务模式或以管理员身份运行 Clash Mimo For Windows' };
            }
