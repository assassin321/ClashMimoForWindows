...
module.exports = function registerBackupHandlers(context) {
    ...
    try {
      ...
      const { filePath, canceled } = await dialog.showSaveDialog({
        ...
        defaultPath: path.join(
          os.homedir(),
          'Downloads',
          `clashmeta_backup_${new Date().toISOString().replace(/:/g, '-').split('.')[0]}.zip`
        ),
        filters: [
  ...
  ipcMain.handle('backup-webdav-upload', async (event, backupType = 'CONFIG_ONLY') => {
    try {
      ...
      const config = {
        ...
        backupDirectory: dbManager.getSetting('webdav_backup_dir', 'Clash Mimo For Windows'),
        fileName: dbManager.getSetting('webdav_backup_filename', 'clashmeta_backup.zip')
      };
      ...
      const tempFilePath = path.join(tempDir, `clashmeta_temp_${Date.now()}.zip`);
  ...
  ipcMain.handle('backup-webdav-download', async (event, fileName = null) => {
    try {
      ...
      // 创建临时文件路径
      const tempDir = os.tmpdir();
      const tempFilePath = path.join(tempDir, `clashmeta_download_${Date.now()}.zip`);
  ...
  ipcMain.handle('backup-webdav-save-config', async (event, config) => {
    try {
      ...
      dbManager.setSetting('webdav_backup_dir', config.backupDirectory || 'Clash Mimo For Windows');
      dbManager.setSetting('webdav_backup_filename', config.fileName || 'clashmeta_backup.zip');
