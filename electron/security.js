...
const ALLOWED_USERAGENTS = {
  ...
  'Clash Mimo For Windows': 'Clash Mimo For Windows/1.0.0' // 将在使用时动态替换为实际版本
};
...
function getSafeUserAgent(uaKey, appVersion) {
  // 如果是Clash Mimo For Windows，动态设置版本号
  if (uaKey === 'Clash Mimo For Windows' && appVersion) {
    return `Clash Mimo For Windows/${appVersion}`;
  }
