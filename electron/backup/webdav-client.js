...
class WebDAVClient {
   ...
   * @param {string} config.backupDirectory - 备份目录（默认"Clash Mimo For Windows"）
   * @param {string} config.fileName - 备份文件名（默认"clashmeta_backup.zip"）
   */
  constructor(config) {
    this.config = config;
    this.client = null;
   ...
   */
  getBackupDir() {
    let dir = (this.config.backupDirectory || 'Clash Mimo For Windows').trim() || 'Clash Mimo For Windows';
    // 移除开头的斜杠（如果有）
    if (dir.startsWith('/')) {
      dir = dir.substring(1);
    }
  ...
  getBackupFilePath(fileName = null) {
    const dir = this.getBackupDir();
    const name = fileName || this.config.fileName || 'clashmeta_backup.zip';
    // 确保路径格式正确
    return `/${dir}/${name}`;
  }
  ...
  /**
   * 生成带时间戳的备份文件名
   * @returns {string} 带时间戳的文件名，如 clashmeta_backup_20251026_143022.zip
   */
  generateTimestampedFileName() {
    ...
    return `clashmeta_backup_${timestamp}.zip`;
  }
  ...
  async uploadBackup(localFilePath, onProgress = null, useTimestamp = true) {
    try {
      ...
      // 生成文件名（带时间戳或固定名称）
      const fileName = useTimestamp ? this.generateTimestampedFileName() : (this.config.fileName || 'clashmeta_backup.zip');
