...
class BackupManager {
  ...
  backupUiSettings() {
    ...
    try {
      ...
      settings.userAgent = `Clash Mimo For Windows/Desktop/${this.context.get('appVersion')}`;
  ...
  backupWebDAVSettings() {
    ...
    try {
      ...
      settings.backupDirectory = this.dbManager.getSetting('webdav_backup_dir', 'Clash Mimo For Windows');
      settings.fileName = this.dbManager.getSetting('webdav_backup_filename', 'clashmeta_backup.zip');
