...
const SERVICE_MODE_SOCKET_PATH = '/tmp/clashmeta-mihomo-service.sock';
...
function getMihomoSocketPath() {
  if (process.platform === 'win32') {
    ...
    return `\\\\.\\pipe\\Clash Mimo For Windows\\mihomo-${sessionId}-${processId}`;
  } else {
    // Unix Domain Socket
    if (useServiceMode) {
    ...
    const processId = process.pid;
    const socketDir = path.join(os.tmpdir(), `clashmeta-${uid}`);
    try {
