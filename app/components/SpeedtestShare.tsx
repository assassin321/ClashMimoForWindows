...
export default function SpeedtestShare({
    ...
    const generateImage = async () => {
      ...
      try {
        ...
        const drawShareCard = () => {
          ...
          ctx.fillStyle = '#1E293B';  // 深蓝灰色
          ctx.fillText('Clash Mimo 网络测速', 180, 85);
          ...
          ctx.fillText(`Clash Mimo For Windows © ${new Date().getFullYear()}`, canvas.width / 2, canvas.height - 30);
        };
  ...
  const handleShare = async () => {
    try {
      ...
      if (navigator.share && navigator.canShare) {
        ...
        // 检查是否支持分享文件
        const shareData = {
          title: 'Clash Mimo For Windows测速结果',
  ...
  const handleDownload = () => {
    ...
    link.href = shareUrl;
    link.download = 'Clash Mimo For Windows-测速结果.png';
