...
/**
 * Windows: 确保 clashmeta-helper.exe 始终使用最新源码构建
 * - 如果本机安装了 Go，并且存在 native/helper 目录，则每次 prepare 都会重新编译
 * - 编译结果写入 native/helper/clashmeta-helper.exe，并同步到 tools/clashmeta-helper.exe
...
function resolveHelper() {
  ...
  const toolsHelperPath = path.join(cwd, 'tools', 'clashmeta-helper.exe')
  ...
  try {
    ...
    console.log('[Helper] Go toolchain not found, using existing tools/clashmeta-helper.exe')
    return
  }

  try {
    console.log('[Helper] Building clashmeta-helper.exe from native/helper ...')
    // 在源码目录生成一个 helper，可根据需要扩展为多架构构建
    execSync('go build -ldflags="-s -w" -o clashmeta-helper.exe .', {
      cwd: helperDir,
      stdio: 'inherit'
    })

    // 将最新构建同步到 tools 目录，供 electron-builder extraResources 使用
    const builtHelperPath = path.join(helperDir, 'clashmeta-helper.exe')
    fs.mkdirSync(path.dirname(toolsHelperPath), { recursive: true })
    fs.copyFileSync(builtHelperPath, toolsHelperPath)
    console.log('[Helper] Updated tools/clashmeta-helper.exe')
  } catch (e) {
