// Clash Mimo For Windows Helper Service
...
const (
	serviceName       = "ClashMetaHelperService"
	serviceDisplay    = "Clash Mimo For Windows Helper Service"
	serviceDesc       = "Clash Mimo For Windows Helper Service for TUN mode"
	pipeName          = `\\.\pipe\clashmeta-helper-service`
	messageExpirySecs = 30
	secretSeed        = "clashmeta-helper-service-secret-key-v1"
)
...
const (
	keyFileDir = "Clash Mimo For Windows"
...
func main() {
	...
	if isService {
		...
		fmt.Println("Clash Mimo For Windows Helper Service")
		fmt.Println("Usage:")
...
func handleCommand(conn net.Conn, req *IpcRequest) {
	...
	case CmdGetVersion:
		data := VersionData{Service: "Clash Mimo For Windows Helper Service", Version: "1.0.0"}
		sendResponse(conn, req.ID, true, data, "")
...
func getAllowedCoreDirs() []string {
	...
	if appData := os.Getenv("APPDATA"); appData != "" {
		dirs = append(dirs, filepath.Join(appData, "Clash Mimo For Windows", "cores"))
		dirs = append(dirs, filepath.Join(appData, "clashmeta", "cores"))
	}
	if localAppData := os.Getenv("LOCALAPPDATA"); localAppData != "" {
		dirs = append(dirs, filepath.Join(localAppData, "Clash Mimo For Windows", "cores"))
		dirs = append(dirs, filepath.Join(localAppData, "clashmeta", "cores"))
	}

	// 3. 所有用户的 profile 目录下的 Clash Mimo For Windows/cores
	...
	for _, usersDir := range usersDirs {
		if entries, err := os.ReadDir(usersDir); err == nil {
			for _, e := range entries {
				if e.IsDir() {
					dirs = append(dirs,
						filepath.Join(usersDir, e.Name(), "AppData", "Roaming", "Clash Mimo For Windows", "cores"))
					dirs = append(dirs,
						filepath.Join(usersDir, e.Name(), "AppData", "Roaming", "clashmeta", "cores"))
					dirs = append(dirs,
						filepath.Join(usersDir, e.Name(), "AppData", "Local", "Clash Mimo For Windows", "cores"))
					dirs = append(dirs,
						filepath.Join(usersDir, e.Name(), "AppData", "Local", "clashmeta", "cores"))
				}
			}
		}
	}
	...
	if pd := os.Getenv("ProgramData"); pd != "" {
		dirs = append(dirs, filepath.Join(pd, "Clash Mimo For Windows", "cores"))
	}
	...
	if runtime.GOOS == "linux" {
		...
		if u, err := user.Current(); err == nil {
			...
			dirs = append(dirs, filepath.Join(u.HomeDir, ".local", "share", "clashmeta", "cores"))
		}
		...
		if entries, err := os.ReadDir("/home"); err == nil {
			for _, e := range entries {
				if e.IsDir() {
					...
					dirs = append(dirs,
						filepath.Join("/home", e.Name(), ".local", "share", "clashmeta", "cores"))
				}
			}
		}
	}
