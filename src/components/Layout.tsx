...
declare global {
  interface Window {
    __clashmetaPendingUpdate?: UpdateEventDetail;
  }
}
...
export default function Layout({ children }: LayoutProps) {
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => {
    ...
    const storedState = window.localStorage.getItem('clashmeta-sidebar-collapsed');
  ...
  const handleToggleSidebar = useCallback(() => {
    setSidebarCollapsed(prev => {
      const next = !prev;
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('clashmeta-sidebar-collapsed', next ? 'true' : 'false');
      }
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const handler = (event: Event) => {
      ...
      if (detail?.release && detail?.currentVersion) {
        ...
        window.__clashmetaPendingUpdate = undefined;
      }
    };

    window.addEventListener(UPDATE_AVAILABLE_EVENT, handler as EventListener);

    const pending = window.__clashmetaPendingUpdate;
          ...
          >
            ...
            {!sidebarCollapsed && (
              <div className="leading-tight">
                <span className="block text-sm font-semibold text-foreground">Clash Mimo For Windows</span>
