...
export default function SubscriptionConverter() {
  ...
  const [fetchUserAgent, setFetchUserAgent] = useState('Clash-Meta-Converter/1.0');
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setFetchUserAgent(result.settings.userAgent || 'Clash-Meta-Converter/1.0');
      }
    } catch (error) {
  ...
  const createSubscriptionAuto = async (sourceContent: string, proxyCount: number) => {
    try {
      ...
      const result = await window.electronAPI.converter.createSubscription({
        name: `Clash Mimo For Windows_${targetFormat}_${Math.floor(Date.now() / 1000)}`,
  ...
  const handleExport = () => {
    ...
    a.download = `clashmeta_${targetFormat}_${Date.now()}.${ext}`;
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  placeholder="Clash-Meta-Converter/1.0"
                  className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <p className="text-xs text-gray-500 dark:text-gray-400">
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                  {[
                    { label: '默认 (Clash Mimo For Windows)', value: 'Clash-Meta-Converter/1.0' },
